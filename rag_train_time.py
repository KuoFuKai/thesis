...
docs = loader.load()
load_end = time.time()
print(f"Loading documents took {load_end - load_start:.2f} seconds")
...
print(f"Transforming documents took {transform_end - transform_start:.2f} seconds")
...
split_end = time.time()
print(f"Splitting documents took {split_end - split_start:.2f} seconds")
...
index_end = time.time()
print(f"Building FAISS index took {index_end - index_start:.2f} seconds")

# 保存向量索引
save_start = time.time()
db.save_local("vector_db")
save_end = time.time()
print(f"Saving FAISS index took {save_end - save_start:.2f} seconds")
